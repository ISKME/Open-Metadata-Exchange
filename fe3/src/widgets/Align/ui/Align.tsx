...
export function AlignWidget({ onSelect = (tag) => {} }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios('/standards/align-widget-helper');
  ...
  const handleFrameworkChange = async (event) => {
    ...
    const result = await axios('/standards/align-widget-helper', { params: {
      field,
      [field]: selected,
    } });
  ...
  const handleFrameworkTagChange = async (event) => {
    ...
    const frameworkData = {
      ...
      area: frameworkAreas.find((a) => a.value === frameworkArea)?.label,
      tag: frameworkTags.find((t) => t.value === selected),
    };
    onSelect(frameworkData);
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: '16px', margin: '16px', minWidth: '500px' }}>
      <FormControl>
