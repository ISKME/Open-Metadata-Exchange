...
const FiltersCheckbox = ({ id = 0, onChange = (id) => {} }) => {
  ...
  let component = (
    <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 15 14" fill="none" onClick={() => setChecked(true)} style={{ cursor: 'pointer' }}>
      <rect x="1" y="0.5" width="13" height="13" rx="2" fill="#FCFCFC" stroke="#979797" />
    </svg>
  );
  if (checked) {
    component = (
      ...
      >
        <rect className="rect" fill="#265BC1" height="13" rx="2" stroke="#265BC1" width="13" x="1" y="0.5" />
        <path
          className="path"
...
export const Dropdown = ({ onAdd = (collectios) => {} }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('/api/imls/v2/collections/site-collections/picker/', {
        params: {
          per_page: '30',
          page: PAGE,
          unshared: true,
      ...
      if (PAGE === 1) {
        ...
        const data = await axios.get('/api/imls/v2/collections/site-collections', {
          params: {
            per_page: '90',
            'f.search': searchTerm,
          },
  ...
  const scrollDiv = (amount) => {
    ...
    if (divRef.current.scrollTop + divRef.current.offsetHeight >= divRef.current.scrollHeight - 70) {
      PAGE++;
      fetchData();
    }
  };
  ...
  return (
    <div className={cls.filters}>
      <div className={cls.searchCollections}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => search()}>
