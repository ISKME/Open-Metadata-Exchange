...
export function ReportsCaseAnalytics() {
  ...
  const fetchData = (startDate, endDate, users = [], group = null, organization = null, filteredIds = []) => {
    const params = {
  ...
  const updateFilters = (key, value) => {
    ...
    const safeEquals = (a, b) => {
      if (typeof a === 'object' && a?.value) a = a.value;
      if (typeof b === 'object' && b?.value) b = b.value;
      return a === b;
    };
    ...
    if (
      updatedParams.subject ||
      updatedParams.grade ||
      updatedParams.area ||
      updatedParams.ethnicity
    ) {
    ...
    fetchData(
      date[0],
      date[1],
      usersParam.length === 0 ? [] : usersParam,
      updatedParams.group,
      updatedParams.organization,
      filteredIds
    );
