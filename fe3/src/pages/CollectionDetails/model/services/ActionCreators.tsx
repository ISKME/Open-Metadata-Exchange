...
export const fetchCollectionDetailsResources = (path) => async (dispatch: AppDispatch) => {
  try {
...
export const fetchFilters = (path) => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch(collectionDetailsSlice.actions.filtersFetchingSuccess(dataPromise.resources.filters));
  } catch (e) {
    dispatch(collectionDetailsSlice.actions.resourcesFetchingError(e.message));
  }
};
...
export const loadFilteredResources = (path) => async (dispatch: AppDispatch) => {
  try {
    ...
    const dataPromise = await promise
      .then((res: { data: any; }) => res.data.resources.items);
    dispatch(collectionDetailsSlice.actions.filteredResourcesFetchingSuccess(dataPromise));
  } catch (e) {
    dispatch(collectionDetailsSlice.actions.resourcesFetchingError(e.message));
  }
};
