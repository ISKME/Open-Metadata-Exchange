...
export default function ArkSearch() {
  ...
  const fetcher = async (queries) => {
    ...
    try {
      const { data } = await api.get(`/api/search/v2/browse/${queries}`);

      document.title = data?.clientInfo?.name
        ? `Search | ${data.clientInfo.name}`
        : 'Search | Digital Public Goods Library';
        ...
        <div className={cls.resourcesList}>
          ...
          <ul>
            {resources && resources.map((el, i) => {
              return !el ? '' : <ArkResourceCard resource={el} key={el?.id || i} />
            })}
          </ul>
