          ...
          {data.children.map((item, j) => (
            <Typography
              sx={{
                ...
                padding: '4px 0',
                cursor: 'pointer',
                backgroundColor: selected.findIndex((select) => select.slug === item.slug) < 0 ? '' : '#ececec',
              }}
...
export function CasesAll() {
  const { cases, count, pages, sorts, order, topics, grades } = useAppSelector((state) => state.CasesSlice);
  ...
  React.useEffect(() => {
    axios.get('/standards/list-existing/standard').then(({ data }) => {
  ...
  const applyFilter = (topic) => {
    ...
    if (index < 0) { // add topic
      ...
      if (topic.parent) tempTopics.push(topics.find((item) => item.slug === topic.parent))
    }
  ...
  const applyFrameworkFilter = (framework) => {
    const index = filteredFrameworks.findIndex((item) => item.id === framework.id)
  ...
  React.useEffect(() => {
    ...
    if (!first && (grade && grades.length || subject && topics.length)) {
      first = true;
      const parent = topics.find((item) => item.children.findIndex((child) => child.slug == subject) > 0);
      ...
      if (parent) {
        applyFilter(parent);
        let temp = parent.children.find((item) => item.slug === searchParams.get('f.general_subject'))
      ...
      <Grid item xs={4} sx={{ paddingRight: '32px' }}>
        ...
        >
          ...
          <AccordionDetails sx={{ padding: '8px 0 0' }}>
            {topics.map((topic, j) => (
              ...
              // </Typography>
              <Subject key={topic.slug} data={topic} onSelect={(item) => applyFilter(item)} selected={filteredTopics} />
            ))}
          </AccordionDetails>
        </Accordion>
        <Accordion
        ...
        <Accordion sx={styles.accordion}>
          ...
          <AccordionDetails sx={{ paddingLeft: 0, paddingRight: 0 }}>
            {frameworks.map((item) => (
              <Accordion key={item.id} sx={styles.sub} onClick={() => applyFrameworkFilter(item)}>
                <AccordionSummary sx={styles.inner}>
                  <Typography>{item.name} ({item.count})</Typography>
                </AccordionSummary>
              </Accordion>
            ))}
          </AccordionDetails>
