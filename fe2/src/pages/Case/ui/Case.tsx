  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        {framework ? <Typography variant="body2" color="text.secondary" sx={{ marginTop: '8px' }}>
  ...
  return (
    <Dialog onClose={() => onClose(/*selectedValue*/)} open={open}>
      ...
      <DialogActions>
        ...
        <Button onClick={handleSelect} autoFocus>Agree</Button>
      </DialogActions>
    </Dialog>
  );
}
...
const post = (url, body) => new Promise((res, rej) => {
  axios.get('/api/csrf-token').then(({ data }) => {
    const headers = { 'Content-Type': 'multipart/form-data', 'X-Csrftoken': data.token }
    axios({ method: 'POST', url, headers, data: { ...body, csrfmiddlewaretoken: data.token } }).then(({ data }) => res(data)).catch(rej)
  }).catch(rej)
})
...
function findSectionIndex(htmlString, charIndex) {
  ...
  const sections = Array.from(htmlString.matchAll(/<[^>]+>.*?<\/[^>]+>/g)).map(m => m[0]);
...
export function Case() {
  ...
  React.useEffect(() => {
    ...
    axios.get('/api/courseware/v1/lessons/' + id).then(({ data }) => {
      const { title, abstract, thumbnail, general_subjects, grades, sections, alignment_tags, annotation_uri } = data;
      setTitle(title);
      ...
      function oncontroldown(event) {
        setActive(-1)
        quote = this.text
        const range = getStartEndPositions(sections[1].content.trim().replace(/<[^>]+>/g, '').replace(/\n/g, '').replace(/&#39;/g, "'"), quote.trim().replace(/\n/g, ''))
        setTextRange(range)
      ...
      for (const tag of alignment_tags) {
        if (frameworks.findIndex((item) => item.name === tag.standard.name) < 0)
          frameworks.push({ ...tag.standard, children: [] })
      }
      for (const tag of alignment_tags) {
        const index = frameworks.findIndex((item) => item.name === tag.standard.name)
      ...
      axios.get('/api/annotations?uri=' + annotationUri).then(({ data }) => {
        ...
        annotations?.filter((item) => !!item.quote)?.forEach((item) => {
          const i = findSectionIndex(sections[1].content, item.end_position)
          if (i < 0) return
  ...
  const publish = (isPrivate = false, text = '', isChecked = false, start, end, tag) => {
    const start_position = isChecked ? 0 : timeToSeconds(start || 0, 'Start')
    const end_position = isChecked ? duration : timeToSeconds(end || duration, 'End')
  ...
  return (
    <Box sx={{ width: '100%', padding: '64px 0' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', height: '70px', backgroundColor: '#fefefe', padding: '0 10% 36px', '@media (maxWidth: 1168px)': { padding: '0 16px' } }}>
        ...
        <CButton href={`/admin/courseware/lesson/${id}/change/`}>Edit in Admin</CButton>
        <div>
          <CButton
            id="fade-button"
      ...
      <Grid container spacing={2} sx={{ padding: '0 10%', '@media (maxWidth: 1168px)': { padding: '0 16px' } }}>
            ...
            <div>
              ...
              {tagForm ? (
                <Card>
                      ...
                      <Grid item xs={12}>
                        <div>
                          <FormControlLabel
                            control={
                              <Checkbox
                                checked={checkedTag}
                                onChange={({ target }) => setCheckedTag(target.checked)}
                        ...
                        <div style={{ display: 'flex', gap: '8px', margin: '8px 0' }}>
                          <TextField
                            ...
                            size="small"
                            value={formStartTag}
                            onChange={({ target }) => setFormStartTag(target.value)}
                            disabled={checkedTag}
                          ...
                          <TextField
                            ...
                            size="small"
                            value={formEndTag}
                            onChange={({ target }) => setFormEndTag(target.value)}
                            disabled={checkedTag}
                        ...
                        <TextField
                          ...
                          style={{ width: '100%' }}
                          value={formTextTag}
                          onChange={({ target }) => setFormTextTag(target.value)}
                        />
                        </div>
                      </Grid>
                    </Grid>
                    <CButton onClick={() => setAlignOpen(true)}>{selectedTag.label || 'Framework Tag'}</CButton>
                    <AlignDialog
                      open={alignOpen}
                  ...
                  <CardActions>
                    <CButton
                      disabled={!checkedTag && (matchTimeFormat(formEndTag) || matchTimeFormat(formStartTag))}
                      onClick={() => publish(false, formTextTag, checkedTag, formStartTag, formEndTag, selectedTag)}
                    >
                      Publish
                    </CButton>
                    <CButton
                      disabled={!checkedTag && (matchTimeFormat(formEndTag) || matchTimeFormat(formStartTag))}
                      onClick={() => publish(true, formTextTag, checkedTag, formStartTag, formEndTag, selectedTag)}
                    >
                      Draft
                    </CButton>
        ...
        <Grid item xs={8}>
          <div style={{ padding: '10px', backgroundColor: '#303e47' }}>
            ...
            {video ? <Player entryId={video?.url?.match(/entry_id=(.+?)\&/)?.[1]} onLoad={(data) => {
              ...
              let notes = annotations
                ...
                ?.filter((item) => (item.end_position <= duration && item.start_position >= 0 && item.start_position <= duration && item.end_position >= 0))
              notes = categorizeByTimeRange(notes ,range)
              notes = notes.map((item) => ({ data: item, position: item[0].end_position / duration * 98 + '%' }))
              setNotes(notes)
            }} onFail={() => {}} /> : ''}
          ...
          {selectedNotes.length ? (
            <div style={{
              ...
              {selectedNotes.map((item) => <Note key={item.id} name={item.user?.first_name ? `${item.user.first_name} ${item.user.last_name}` : item.user.email} text={item.text} date={item.created} framework={item.alignment_tag} />)}
            </div>
          ) : ''}
          {form ? (
            <Card sx={{ minWidth: 275 }}>
                ...
                <Grid container sx={{ margin: '16px 0' }}>
                  <Grid item xs={6}>
                    ...
                    <TextField
                      ...
                      style={{ width: '100%' }}
                      value={formTextVideo}
                      onChange={({ target }) => setFormTextVideo(target.value)}
                    />
                    </div>
                  </Grid>
                  <Grid item xs={6}></Grid>
                </Grid>
                <CButton onClick={() => setAlignOpenVideo(true)}>{selectedTagVideo.label || 'Framework Tag'}</CButton>
                <AlignDialog
                  open={alignOpenVideo}
              ...
              <CardActions>
                <CButton
                  ...
                  onClick={() => publish(false, formTextVideo, checked, formStart, formEnd, selectedTagVideo)}
                >
                  Publish
                </CButton>
          ...
          <div hidden={value !== 0}>
            <div className="links">
              ...
              <div className="how-to-wrapper visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block">
                <button type="button" className="case-link how-to-link" data-trigger="hover focus" data-original-title="" title="">
                ...
                if (item.startsWith('<p')) {
                  return (
                    <div key={index} style={{
                      ...
                      <div style={{ flex: 1 }} dangerouslySetInnerHTML={{ __html: item.replace('<p', '<p class="comment"') }}></div>
                      <div style={{ flex: (active === index) && notesByComments[active] ? 1 : '' }}>
                        {active === index ? (
                          <div>
                          ...
                          <div>
                            ...
                            {notesByComments[index]?.length ? <span>({notesByComments[index]?.length})</span> : ''}
          ...
          <div hidden={value !== 1}>
            <div className="links">
              ...
              <button className="btn btn-link pull-right show-popup-form-button mod-material-note js-show-material-note-form" title="Add a note about Instructional Materials">
            ...
            {annotations?.filter((item) => !item.quote)?.length ? (
              <div style={{
                ...
                <div style={{
                  ...
                  <h4>Video Notes ({annotations?.filter((item) => !item.quote)?.length})</h4>
                </div>
                {annotations?.filter((item) => !item.quote)?.map((item) => <Note key={item.id} name={item.user?.first_name ? `${item.user.first_name} ${item.user.last_name}` : item.user.email} text={item.text} date={item.created} framework={item.alignment_tag} />)}
              </div>
            ) : ''}
          </div>
        </Grid>
