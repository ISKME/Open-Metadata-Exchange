    ...
    <div className={classNames(cls.search_resources, {}, [className])}>
      ...
      {!done && (<div>
        ...
        <div className={cls.search_block}>
          <SearchBar value={inputValue} onInputChange={handleInputChange} placeholder="Search Individual Learning Materials" onKeyDown={onPress} onClear={() => update(null)} />
          <Button onClick={handleSearching} title="Search" className={cls.search_block_button} />
          <Select options={sortBy} onSelectChange={handleSelectChange} initial={sorted} />
        </div>
          ...
          <div className={cls.search_resources_list}>
            ...
            {(!isLoading && (resources && resources.length === 0)) && (
              <div className={cls.loaderContainer}>
                <span style={{ fontSize: '18px', color: '#474F60' }}>No materials found!</span>
                <span style={{ fontSize: '16px', color: '#474F60' }}>Try adjusting your search or filter to find what you are looking for.</span>
              </div>
            )}
            {isLoading && (
              <div className={cls.loaderContainer}>
            ...
            <div className={cls['frame']}>
              <div className={cls['div']}>
                ...
                <div className={cls['frame-3']}>
                  ...
                  <p className={cls['any-changes-pushed']}>
                    <span className={cls['text-wrapper-4']}>Any changes pushed to this content by </span>
                    <span className={cls['text-wrapper-5']}>{collectionMicrositeName}</span>
                    <span className={cls['text-wrapper-4']}> are automatically synced to </span>
              ...
              {!map && (
                <div className={cls['frame-4']}>
                  <CollectionItemCard collection={{ micrositeName: collectionMicrositeName, numResources, thumbnail, name: collectionName, micrositeSlug, id: '', educationLevels }} />
                  {ready && (
                    <div className={cls['frame-5']}>
                      <div className={cls['button']} onClick={() => /*navigate('/imls/site-collections/subscribed-preferences/#meta')*/setMap(true)}><p className={cls['label-text']}>Map Metadata in the OER Exchange</p></div>
                    </div>
                  )}
                  {!ready && (
                    <>
                      <div className={cls.progress}>
                        <div className={cls.progressBar} style={{  width: `${progress * 100}%` }}></div>
            ...
            {sections.map((section) => (
              <Collapse number={stats[section.name]} title={section.name}>
                <div className={cls['preferences-values']}>
                  ...
                  >
                    <div className={cls['label-text']} style={{ color: 'white' }}>Save and Update Map</div>
                  </button>
                </div>
              </Collapse>
            ))}
