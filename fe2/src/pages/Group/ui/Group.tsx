...
export function Group() {
  ...
  React.useEffect(() => {
    axios.get(`/api/groups/v1/groups/${id}.json`).then(({ data }) => {
      let i = 0;
      const { title, cover,description, members_count, folders, members } = data;
      setTitle(title);
      setCover(cover || customUrl);
      setMembersCount(members_count);
      setFolders(folders);
      setMembers(members.map((item) => ({ ...item, id: i++, joined: new Date(item.joined) })));
      ...
      <section className={cls['group-info']}>
        <div className={cls['group-info-container']}>
          <div className={cls['group-info-cover']}>
            <img src={cover} width="150" height="150" alt={title} onError={({ target }) => { target.src = 'https://semantic-ui.com/images/wireframe/image.png' }} />
          </div>
          <div className={cls['group-info-text']}>
            ...
            <div className={cls['group-info-counters']}>
              ...
              <div className={cls['group-info-counters-item']}>
                ...
                <span className={cls.text}>
                  Cases:
                  <span className={cls['text-counter']} style={{ marginLeft: '6px' }}>86</span>
                </span>
              </div>
            </div>
          </div>
