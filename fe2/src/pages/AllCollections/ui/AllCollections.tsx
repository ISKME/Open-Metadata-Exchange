...
export function AllCollections({ className }: AllCollectionsProps) {
  ...
  const [description, setDescription] = useState<any>(<>All of the collections shared to OER Exchange</>);
  ...
  const hasFilter = () => Object.entries(qs.parse(search)).filter((item) => item[0] !== 'tenant').filter((item) => item[1].length).length;
  const hasTenant = () => Object.entries(qs.parse(search)).filter((item) => item[0] === 'tenant').filter((item) => item[1].length).length;
    ...
    <div className={classNames(cls.allcollections, {}, [className])}>
      <div className={cls['breadcrumb']}>
        <div className={cls['breadcrumbs']}>
          <Breadcrumb first="Explore" middle="All Collections" last={title === 'All Collections' ? '' : tenantTitle} />
        </div>
              ...
              if (tenantTitle) {
                ...
                if (params.tenant) {
                  ...
                  axios.get('/api/imls/v2/microsites/' + params.tenant[0]).then(({ data }) => {
                    ...
                    setDescription(<>All of the collections shared to OER Exchange by <a href={`${data.url}`}>{data.name}</a></>);
                  });
                }
              }
            } else {
              if (tenantTitle) {
                ...
                if (params.tenant) {
                  axios.get('/api/imls/v2/microsites/' + params.tenant[0]).then(({ data }) => {
                    ...
                    setTenantUrl(data.url);
                    setTitle(data.name + ' Collections');
                    setDescription(<>All of the filtered collections shared to OER Exchange by <a href={`${data.url}`}>{data.name}</a></>);
                  });
                }
              }
            }
