...
function Button({ children, onClick, selected }) {
  const selectedStyle = selected ? { background: 'rgb(55, 58, 72)', color: 'white' } : {}
  return <button style={{ ...style, ...selectedStyle }} onClick={onClick}>{children}</button>;
}

const fetchPath = (path, callback) => (pageNumber: number, pageSize: number = 9) => new Promise((resolve) => {
...
export const Paging = ({ path = '', items = null, query = false, children = null, itemCount = 11, pageSize = 9, styles = {}, done = (count, filters, sortByOptions) => {} }) => {
  ...
  if (path) return <div>
    <Component fetchPage={fetchPage} pageSize={pageSize} itemCount={itemCount} items={items} key={key}>
      {
        (items, { currentPage, pageCount, pages }, { goto, back, next, first, last }) => (
          <div>
            ...
            {itemCount / pageSize > 1 && <div>
              ...
              {(currentPage >= 4) && <Button onClick={first}>1</Button>}
              {(currentPage > 4) && <span>...</span>}
              {pages.map((p) => (Math.abs(currentPage - p) < 4) && <Button onClick={() => goto(p)} key={p} selected={p === currentPage}>{p === currentPage ? <b>{p + 1}</b> : p + 1}</Button>)}
              {(pageCount - currentPage > 5) && <span>...</span>}
              {(pageCount - currentPage >= 5) && <Button onClick={last}>{pageCount}</Button>}
              <Button onClick={next}>&gt;</Button>
              <Button onClick={last}>&gt;&gt;</Button>
