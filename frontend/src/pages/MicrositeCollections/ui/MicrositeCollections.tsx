  ...
  return (
    <div className={cls.navigation}>
      <div className={cls.frame}>
        <Link to="/imls/explore-oer-exchange">
          <div className={cls.div}>
            <div className={cls['frame-wrapper']}>
              <div className={cls.div}>
                <svg className={cls.vector} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        <svg className={cls.img} xmlns="http://www.w3.org/2000/svg" width="2" height="24" viewBox="0 0 2 24" fill="none">
...
export function MicrositeCollections({ className }: AllCollectionsProps) {
  ...
  const [path, setPath] = useState(`collections/browse/${window.location.search.toString()}`);
  ...
  const fetcher = async () => {
    try {
      const { data } = await axios.get(`/api/imls/v2/collections/browse`);
  ...
  useEffect(() => {
    ...
    axios.get('/api/imls/v2/microsites/' + name).then(({ data }) => {
      setTitle(data.name);
  ...
  return (
    <div className={classNames(cls.allcollections, {}, [className])}>
      <div className={cls['breadcrumb']}>
        ...
        <MagnifyingGlassSolid className={cls['square-up-right']} color="#1E1E1E" />
      </div>
      {/* <ArrowIcon /> */}
      ...
      <h5 className={cls.allcollections_desc}>All of the collections shared to the OER Exchange by <a href={`${tenantUrl}`}>{title}</a></h5>
      <div className={cls.search_block}>
        <SearchBar value={inputValue} onInputChange={handleInputChange} placeholder="Search Collections" onKeyDown={onPress} onClear={() => updatePath(null)} />
      ...
      <div className={cls.allcollections_board}>
        <ul className={cls.filter_bar}>
          ...
          <Button onClick={handleClearSearchParams} title="Reset" className={cls.reset} />
        </ul>
        <Paging
          key={path}
          path={path}
        ...
        >
          {(el, i) => {
            ...
            return <CollectionItemCard key={`${data.id}-${i}`} collection={data} />;
          }}
        </Paging>
        {notFound && (
