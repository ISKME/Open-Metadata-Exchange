...
export function SearchResources({ className }: search_resourcesProps) {
  ...
  const fetcher = async (queries) => {
    ...
    try {
      const { data } = await axios.get(`/api/imls/v2/resources/${queries}`);
  ...
  return (
    <div className={classNames(cls.search_resources, {}, [className])}>
      ...
      <div className={cls.search_block}>
        <SearchBar value={inputValue} onInputChange={handleInputChange} placeholder="Search Individual Learning Materials" onKeyDown={onPress} onClear={clearing} />
