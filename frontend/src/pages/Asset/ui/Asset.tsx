...
export function Asset() {
  ...
  useEffect(() => {
    axios.get(`/api/imls/v2/resources/${details}`).then(({ data }) => {
      ...
      setLoading(false);
      // axios.get(micrositeResourceURL).then(html => setUrl(html.match(/\<a[^\>]+view-resource-link[^\>]+\>/)?.[0].match(/href="([^"]*)"/)?.[1]))
    });
  }, []);
      ...
      <Card sx={{
        ...
        <CardContent sx={{ flex: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary" mt={1}>
            First published {create}
          </Typography>
          {!!update && <Typography variant="body2" color="text.secondary" mb={1}>
            Last updated {new Date(update).toDateString().slice(4)}
          </Typography>}
