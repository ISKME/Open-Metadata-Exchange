...
import cls from './MySettings.module.scss';
...
export function MySettings() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        return axios.get('/api/users/v1/profile/edit/');
      })
      .then((response) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post('/api/users/v1/profile/edit/', form, {
        headers: {
      ...
      .catch((error) => {
        if (
          ...
          if (errors.user_errors) {
            for (const key in errors.user_errors) {
              errorMsg += `${key}: ${errors.user_errors[key]}\n`;
            }
          }
          if (errors.profile_errors) {
            for (const key in errors.profile_errors) {
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              ...
              <Button href="/my/new/account/" variant="text">
                Cancel
              </Button>
            </Grid>
          </Grid>
