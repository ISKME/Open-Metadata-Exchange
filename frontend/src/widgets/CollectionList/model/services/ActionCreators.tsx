...
export const fetchLibrary = () => async (dispatch: AppDispatch) => {
  try {
    ...
    const response = await axios.get('/api/imls/v2/explore-oer-exchange/');
...
export const fetchCollections = () => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch(collectionSlice.actions.collectionsFetchingSuccess(response.data.collections.items));
  } catch (e) {
...
export const fetchFilters = () => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch(collectionSlice.actions.filtersFetchingSuccess(dataPromise.filters));
  } catch (e) {
    dispatch(collectionSlice.actions.collectionsFetchingError(e.message));
  }
};

export const loadFilteredCollections = (path) => async (dispatch: AppDispatch) => {
  try {
    dispatch(collectionSlice.actions.collectionsFetching());
    const promise = axios.get(`${URL}${path}`);
    const dataPromise = await promise
      .then((res: { data: any; }) => res.data.collections);
    dispatch(collectionSlice.actions.filteredCollectionsFetchingSuccess(dataPromise.items));
  } catch (e) {
    dispatch(collectionSlice.actions.collectionsFetchingError(e.message));
  }
};

export const loadSharedCollections = (page = 1) => async (dispatch: AppDispatch) => {
  try {
    dispatch(collectionSlice.actions.collectionsFetching());
    const promise = axios.get(`/api/imls/v2/collections/site-collections/picker?per_page=9&page=${page}`);
    const dataPromise = await promise
      .then((res: { data: any; }) => res.data.collections);
    dispatch(collectionSlice.actions.filteredCollectionsFetchingSuccess(dataPromise.items));
  } catch (e) {
...
export const loadSortBy = () => async (dispatch: AppDispatch) => {
  try {
    ...
    const promise = axios.get(URL);
    const dataPromise = await promise
      .then((res: { data: any; }) => res.data.collections.sortByOptions);
