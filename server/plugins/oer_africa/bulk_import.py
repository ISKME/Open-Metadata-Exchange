...
def bulk_import(url: str = API_URL) -> list:
    ...
    if not (raw_path := here / "oer_africa_bulk.json").exists():
        import httpx
    ...
    if not (ome_path := here / "oer_africa_ome_metadata.json").exists():
        payload = json.loads(raw_path.read_text())
        # Drupal JSON:API wraps results in a "data" key; fall back to a bare list
        resources = payload.get("data", payload) if isinstance(payload, dict) else payload
